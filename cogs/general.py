...
class General:
    ...
    async def _help(self, ctx):
        ...
        if ctx.message.guild is not None:
            cmdpf = self.bot.command_prefix(self.bot, ctx.message)
            cmdpf = cmdpf[0]
        ...
        embed.set_footer(icon_url=ctx.message.author.avatar_url, text="https://luki.pw/monika".format(ctx.message.author.name))
        embed.add_field(name="Commands", value="Want to see my commands? Check out my command list [here](https://luki.pw/monika/commands.html)!", inline=False)
        ...
        embed.add_field(name="Donate", value="Want to become a Patron? You can become one by donating [here](https://patreon.com/teamluki) or [here](https://paypal.me/teamluki/5).", inline=False)
    ...
    async def server(self, ctx):
        ...
        embed = embeds.embed(ctx, "Join my server!", "You can join my server [here](https://discord.gg/DspkaRD).")
        await ctx.send(embed=embed)

    @commands.command()
    async def invite(self, ctx):
        """Gives you an invite for Monika!"""
        embed = embeds.embed(ctx, "Invite me!", "You can invite me [here](https://discordapp.com/api/oauth2/authorize?client_id=399315651338043392&permissions=8&scope=bot).")
        await ctx.send(embed=embed)

    @commands.command()
    async def upvote(self, ctx):
        """Gives you the link to upvote Monika."""
        embed = embeds.embed(ctx, "Upvote me!", "You can upvote me [here](https://discordbots.org/bot/monika/vote).")
        await ctx.send(embed=embed)
    ...
    async def donate(self, ctx):
        embed = embeds.embed(ctx, "Donate to me!", "You can support me [here](https://www.patreon.com/teamluki).")
        await ctx.send(embed=embed)
